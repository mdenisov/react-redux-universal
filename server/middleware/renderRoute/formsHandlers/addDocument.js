...
const handler = function* handler({ requestFromFetchAPI }) {
  ...
  const maxID = parseInt(
    documents.documents.document[documents.documents.document.length - 1].id, 10);
  const curDate = new Date();
  documents.documents.document.push({
    id: (maxID + 1),
    displayName: this.request.body.nameDocument,
    docDate: `${curDate.getUTCFullYear()}-${`0${curDate.getUTCMonth() + 1}`.substr(-2)}-` +
      `${`0${curDate.getUTCDate()}`.substr(-2)}`,
  });
